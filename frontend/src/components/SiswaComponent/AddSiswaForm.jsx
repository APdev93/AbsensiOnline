...
const AddSiswaForm = ({ onHide }) => {
	...
	const validateForm = () => {
		let valid = true;
		let errors = { nsin: "", fullName: "", kelas: "", parentNumber: "" };

		if (nsin.trim() === "") {
			errors.nsin = "Nsin tidak boleh kosong";
			valid = false;
		}
		if (fullName.trim() === "") {
	...
	const handleSubmit = async e => {
		...
		try {
			...
			if (data.status_code === 201) {
				...
				setKelas("");
				setNomerWali("");
				setErrors({ nsin: "", fullName: "", kelas: "", parentNumber: "" });
			} else {
				setIsLoading(false);
					...
					<form onSubmit={handleSubmit}>
						<div className="form-group mb-2 input-group-md">
							...
							<input
								...
								required
								onChange={e => {
									setNsin(e.target.value);
									validateForm();
								}}
							/>
							{errors.nsin && <small className="text-danger">{errors.nsin}</small>}
						</div>
						<div className="form-group mb-2 input-group-md">
