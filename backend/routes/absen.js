...
absen.post("/absensi", (req, res) => {
	...
	if (new Date(waktu_absen) <= batas_masuk) {
		...
		const terlambat_menit = Math.floor(
			(new Date(waktu_absen) - batas_masuk) / 60000,
		);
...
absen.post("/absensi-pulang", (req, res) => {
	...
	db.query(sqlCheck, [siswa_id], (err, result) => {
		...
		db.query(
			sqlUpdate,
			[waktu_pulang, statusPulang, result[0].id],
			(err, updateResult) => {
				if (err) {
					return res.status(500).json({ error: err.message });
				}
				res
					.status(200)
					.json({ message: "Absensi pulang tercatat", status: statusPulang });
			},
		);
	});
});
...
absen.get("/absensi/:date", (req, res) => {
