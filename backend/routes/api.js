...
api.post("/siswa/add", verifyAuthToken, (req, res) => {
	...
	const query = `INSERT INTO students (student_id, nsin, full_name, class, parent_number, created_at) VALUES (?, ?, ?, ?, ?, current_timestamp())`;
	let idSiswa = generateStudentCode();
	db.query(query, [idSiswa, nsin, full_name, class_name, parent_number], (error, data) => {
		if (error) {
...
api.post("/siswa/update", verifyAuthToken, (req, res) => {
	...
	db.query(query, [nsin, full_name, class_name, parent_number, sid], (error, data) => {
		if (error) {
			console.error("[ API ] : ERROR => ", error);
			return response("Internal Server Error!", null, 500, res);
